<?php
/**
 * MarketLink - Customer Seller Order Chat
 * Real-time order messaging system between Customer and Farmer
 * Supports text, photos, documents, and auto-closure on order completion
 */

$pageTitle = 'Seller Order Messages';
$activePage = 'chat';
...
<div class="page-header-row" style="margin-bottom:1.5rem;">
  <div>
    <h1 class="page-heading">
      <span>💬</span> Seller Order Chat
    </h1>
    <p class="page-subheading">
      Directly communicate with local farmers, send pickup notes, share photos, and view harvest documents
    </p>
  </div>
</div>
...
<div class="chat-portal-layout" id="chatPortalApp">
  ...
  <aside class="chat-sidebar-panel">
    <div class="chat-sidebar-header">
      <div class="chat-search-box">
        <span>🔍</span>
        <input type="text" id="convSearchInput" placeholder="Search orders, stalls..." autocomplete="off">
      </div>
    </div>
    ...
    <div class="chat-conversations-list" id="convListContainer">
      <div style="padding:2rem 1rem; text-align:center; color:var(--text-muted); font-size:0.875rem;">
        <span>⏳</span> Loading conversations...
      </div>
    </div>
  </aside>
  ...
  <main class="chat-main-window">
    ...
    <div id="chatEmptyState" class="chat-empty-panel">
      ...
      <h3 style="font-size:1.35rem; font-weight:800; color:var(--text-primary); margin-bottom:0.5rem;">Select an Order Conversation</h3>
      <p style="color:var(--text-secondary); max-width:400px; margin:0 auto 1.5rem; font-size:0.9rem; line-height:1.5;">
        Choose a harvest order from the left panel to message the producer directly about pickup details, harvest questions, or receipts.
      </p>
    </div>
    ...
    <div id="chatActiveThread" class="chat-thread-wrapper" style="display:none;">
      ...
      <div class="chat-thread-header">
        <div style="display:flex; align-items:center; gap:0.85rem;">
          ...
          <div>
            ...
            <div style="font-size:0.75rem; color:var(--text-muted); margin-top:2px;" id="threadSubtitle">
              Stall Location &bull; Pickup Date
            </div>
          </div>
        </div>

        <div style="display:flex; align-items:center; gap:0.65rem;">
          <span class="order-status-badge" id="threadStatusBadge">● Placed</span>
          <a href="#" id="threadOrderLink" class="btn-secondary" style="font-size:0.75rem; padding:0.4rem 0.75rem; text-decoration:none;">
            View Order
          </a>
        </div>
      </div>
      ...
      <div id="chatClosedBanner" class="chat-closed-alert" style="display:none;">
        <span>🔒</span>
        <div>
          <strong>Order Ended &bull; Conversation Archived:</strong>
          This pre-order is closed. The messaging thread is preserved in read-only mode for your records.
        </div>
      </div>
      ...
      <div id="attachmentPreviewBox" class="chat-attachment-preview" style="display:none;">
        ...
        <button type="button" id="btnRemoveAttachment" style="background:none; border:none; color:var(--rose-500); cursor:pointer; font-weight:700; font-size:1rem;">
          ✕
        </button>
      </div>
      ...
      <div class="chat-composer-container" id="chatComposerBox">
        <form id="chatMessageForm" style="display:flex; align-items:center; gap:0.65rem; width:100%;">
          ...
          <!-- Text Input -->
          <input type="text" id="chatTextInput" placeholder="Type a message to the farmer..." 
...
<style>
/* Chat Portal Responsive Design & Theme Integration */
.chat-portal-layout {
  display: flex;
...
function renderConversationsList() {
  ...
  filtered.forEach(c => {
    ...
    const unreadHtml = c.unread_count > 0 
      ? `<span style="background:var(--emerald-600); color:#fff; font-size:0.7rem; font-weight:700; padding:2px 7px; border-radius:999px;">${c.unread_count}</span>` 
      : '';

    const statusPill = `<span class="order-status-badge status-${c.order_status}" style="font-size:0.65rem; padding:1px 6px;">● ${c.order_status}</span>`;
...
async function loadMessages(orderId, isPolling = false) {
  try {
    ...
    if (data.status === 'success') {
      ...
      threadStatusBadge.className = `order-status-badge status-${order.order_status}`;
      threadStatusBadge.innerText = `● ${order.order_status.replace('_', ' ').toUpperCase()}`;
      threadOrderLink.href = `<?= BASE_URL ?>/customer/orders.php`;
...
function renderMessagesStream(messages, isPolling) {
  if (messages.length === 0) {
    messagesContainer.innerHTML = `
      <div style="text-align:center; padding:3rem 1rem; color:var(--text-muted); font-size:0.875rem;">
        🌿 No messages in this pre-order chat yet. Ask the seller about harvest status or pickup instructions!
      </div>`;
    return;
  }
  ...
  chatMessageForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        Toast.error('Message Not Sent', data.message || 'Error sending message.');
      }
    } catch (err) {
      Toast.error('Network Error', 'Could not dispatch message to server.');
    } finally {
      btnSend.disabled = false;
      btnSend.innerHTML = '<span>Send</span> ➔';
    }
  });
}
